...
inline std::string SHA256::final() {
    ...
    // std::stringstream yerine doðrudan std::string kullanarak hex dönüþümü yapalým
    std::string result;
    result.reserve(64); // 8 karakter * 8 blok = 64 karakter

    for (auto val : h) {
        char buffer[9]; // 8 karakterlik hex + null terminator
        snprintf(buffer, sizeof(buffer), "%08x", val); // Hex formatýna çevir
        result += buffer;
    }
