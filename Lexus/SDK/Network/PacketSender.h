...
class PacketSender { // // 1.21.2
public:
    CLASS_MEMBER(class networkSystem*, networkSystem, 0x20);
public:
...
{
    CLASS_MEMBER(std::string, realIp, 0x8);
    CLASS_MEMBER(std::string, serverIp, 0x28);
};

class networkSystem {
public:
    CLASS_MEMBER(class RemoteConnectorComposite*, RemoteConnectorComposite, 0x80);
...
public:
    CLASS_MEMBER(class RaknetConnector*, RakNetConnector, 0x60);
};